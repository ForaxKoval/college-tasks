...
Виконайте кожне з наведених нижче завдань за допомогою одного оператора.
...
```
if (fPtr == &number1)
    printf("Адреси збігаються\n");
else
    printf("Адреси не збігаються\n");
```
...
++zPtr;
```
Все окей, помилки немає, цією командою ми зміщуємось на наступний елемент масиву zPtr (тепер zPtr вказує на z[1] )

**б)**
```
/* використання вказівника для отримання першого елемента масиву */
number = zPtr;
```
Помилка, змінній number (яка має тип int) присвоюється вказівник zPtr (а у нього тип int*, тобто адреса а не значення)
Правильно: number = *zPtr; цією командою ми отримаємо значення першого елементу масиву.

**в)**
```
/* присвоєння змінній number значення другого елемента масиву (число 3) */
number = *zPtr[2];
```
Помилка. тому зщо спочатку виконається оператор [] і тому ми будемо намагатись розіменувати якесь число *int. 
Правильно: 
```
...
++z;
```
Помилка. оскільки z це ім'я масиву, а не вказівник. Ім'я масиву не можна змінювати. Правильно: ++zPtr
